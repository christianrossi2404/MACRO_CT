...
# La ruta de la plantilla de Word. Si quieres usar la ruta de tu red
# puedes descomentar la siguiente linea y comentar las otras dos:
word_template_path = r"Y:\COSTES\PLANTILLAS\CARACTERISTICAS TECNICAS.docx"

# Usamos la misma ruta del script para encontrar el archivo de Excel
# ¡IMPORTANTE!: Se ha añadido una 'r' al principio para que Python interprete
# correctamente las rutas de Windows con barras invertidas.
excel_file_path = r"C:\Users\Christian Rossi\PRUEBAS\CT_excel\CO-100-S-8-TRL-E2.xlsx"
# La ruta a la plantilla de Word. Asumimos que está en la misma carpeta
# que el script de Python.
# word_template_path = r"CARACTERISTICAS TECNICAS.docx"
...
def create_ct_document(excel_file, word_template):
    """
    Función principal que lee datos de Excel, procesa la lógica y
    genera el documento de Word.
    """
    try:
        # --- NUEVO BLOQUE DE CÓDIGO: DESENCRIPTADO Y CARGA DEL ARCHIVO EXCEL ---
        # 1. Pide la contraseña de forma segura (el texto no se muestra).
        print(f"Abriendo el archivo: {os.path.basename(excel_file)}")
        password = getpass("Entra la contraseña para el archivo de Excel: ")
        
        # 2. Crea un objeto de flujo de bytes en memoria para guardar el archivo desencriptado.
        decrypted_workbook_stream = io.BytesIO()
        
        try:
            # 3. Abre el archivo de Excel y lo desencripta.
        ...
        # 4. Carga el libro de trabajo desencriptado con openpyxl.
        #    'decrypted_workbook_stream' contiene el archivo en memoria.
        ...
        # Definir las variables tal como en el código VBA
        ManometroAnalogico = ""
        ...
        # Este diccionario mapea los marcadores de la plantilla ({{MARCADOR}}) a los valores
        # que hemos obtenido o calculado.
        context = {
            "NOF": str(b1_value),
        ...
        output_path = os.path.join(os.path.dirname(os.path.abspath(excel_file)), output_filename)
...
if __name__ == "__main__":
    create_ct_document(excel_file_path, word_template_path)
